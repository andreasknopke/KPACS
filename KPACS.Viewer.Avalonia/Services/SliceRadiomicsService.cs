...
public static class SliceRadiomicsService
{
        ...
        out SliceRadiomicsResult result)
    {
        ...
        if (measurement.Kind is not MeasurementKind.RectangleRoi and not MeasurementKind.EllipseRoi and not MeasurementKind.PolygonRoi)
        {
            return false;
        }
        ...
        List<short> samples = measurement.Kind switch
        {
            ...
            MeasurementKind.EllipseRoi => ExtractEllipseSamples(imagePoints, sliceIndex, volume),
    ...
    private static List<short> ExtractEllipseSamples(Point[] imagePoints, int sliceIndex, SeriesVolume volume)
    {
        Point start = imagePoints[0];
        Point end = imagePoints[1];
        ...
        double radiusX = Math.Max((right - left) * 0.5, 0.5);
        double radiusY = Math.Max((bottom - top) * 0.5, 0.5);

        int sampleLeft = Math.Clamp((int)Math.Floor(left), 0, volume.SizeX - 1);
        int sampleRight = Math.Clamp((int)Math.Ceiling(right), 0, volume.SizeX - 1);
        int sampleTop = Math.Clamp((int)Math.Floor(top), 0, volume.SizeY - 1);
        int sampleBottom = Math.Clamp((int)Math.Ceiling(bottom), 0, volume.SizeY - 1);
